    ...
    <Row>
      <Col span="8" offset="2">
        ...
        <Form ref="formDynamic" :model="formDynamic" :label-width="80" style="width: 300px">
          ...
          <p>Message is: {{ this.$store.state.setting.formDynamic.date }}</p>
          <p>Message is: {{ this.$store.state.setting.formDynamic.team }}</p>
          <p>Message is: {{ this.$store.state.setting.formDynamic.items }}</p>
          <FormItem
...
export default {
  ...
  data() {
    return {      
      ...
      date: this.$store.state.setting.formDynamic.date,
  ...
  created() {this.$store.dispatch("getmember");
    // 调用请求菜单列表数据的接口
  ...
  methods: {
    ...
    handleSubmit2() {
      ...
      obj.formDynamic.team =this.team;
      obj.formDynamic.date = moment(this.date).format("YYYY-MM");;
      // obj.member = [];
      // obj.id = "fog";

...
export default {
  ...
  data() {
    return {
      tableData: [],
      columns: [],
      tableDataHandled:  this.$store.state.output.longtabledata, // 带有数据的表格数据
      emptyData: [], // 空的表格数据
      scrollIndex: 1,
      canEdit: true,
      editRow: 1,
  ...
  watch: {
    ...
    tableDataHandled(res) {
      console.log("watch",this.$store.state.output.longtabledata,res)
      return  this.$store.state.output.longtabledata
    }
  },
  methods: {
    handleMoving(e) {
    ...
    //     dataArr.push(arr);
    //   }
      this.tableDataHandled = this.$store.state.output.longtabledata;
    },
    changeDefaultSort() {

...
//1、清除所有数据
router.get('/api/fdata/clrall', async (ctx, next) => {
    let deldata = await new Promise(function (resolve, reject) {
        fdatalib.remove({}, { multi: true }, function (err, numRemoved) {
...
//2、增加一条数据
router.post('/api/fdata/add', async (ctx, next) => {
    ...
    newlib = await new Promise(function (resolve, reject) {
        ...
        slibfind.then(() => {fdatalib.insert(data, function (err, docs) {
...
//3 查询所有数据
router.get('/api/fdata/query', async (ctx, next) => {
    let lib = await new Promise(function (resolve, reject) {
        fdatalib.find({}).exec(function (err, docs) {
            resolve(docs)
        });
    });
    let c = await new Promise(function (resolve, reject) {
        fdatalib.count({}, function (err, count) {
            resolve(count)
        });
    });
...
router.get('/api/fdata/search/', async (ctx, next) => {
    // console.log('收到的查询参数', ctx.request.body)
    console.log('收到的查询参数', ctx.query)//使用ctx.query可以获取？查询的参数
    let lib = await new Promise(function (resolve, reject) {
        fdatalib.find({ 时间: ctx.query["0"] }).exec(function (err, docs) {
            resolve(docs)
        });
    });
    console.log('列表', lib);

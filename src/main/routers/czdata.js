...
//1、清除所有数据
router.get('/api/czdata/clrall', async (ctx, next) => {
    let deldata = await new Promise(function (resolve, reject) {
        czdatalib.remove({}, { multi: true }, function (err, numRemoved) {
...
//2、增加一条数据
router.post('/api/czdata/add', async (ctx, next) => {
    ...
    newlib = await new Promise(function (resolve, reject) {
        ...
        slibfind.then(() => {czdatalib.insert(data, function (err, docs) {
...
//3 查询所有数据
router.get('/api/czdata/query', async (ctx, next) => {
    let lib = await new Promise(function (resolve, reject) {
        czdatalib.find({}).exec(function (err, docs) {
            resolve(docs)
        });
    });
    let c = await new Promise(function (resolve, reject) {
        czdatalib.count({}, function (err, count) {
            resolve(count)
        });
    });
...
router.get('/api/czdata/search/', async (ctx, next) => {
    // console.log('收到的查询参数', ctx.request.body)
    console.log('收到的查询参数', ctx.query)//使用ctx.query可以获取？查询的参数
    let lib = await new Promise(function (resolve, reject) {
        czdatalib.find({ taskid: ctx.query["0"] }).exec(function (err, docs) {
            resolve(docs)
        });
    });
    console.log('列表', lib);

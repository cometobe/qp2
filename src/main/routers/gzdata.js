...
//1、清除所有数据
router.get('/api/gzdata/clrall', async (ctx, next) => {
    let deldata = await new Promise(function (resolve, reject) {
        gzdatalib.remove({}, { multi: true }, function (err, numRemoved) {
...
//2、增加一条数据
router.post('/api/gzdata/add', async (ctx, next) => {
    ...
    newlib = await new Promise(function (resolve, reject) {
        let slibfind = new Promise(function (resolve, reject) {
            slibdata.find({任务编号:data["任务编号"]}).exec(function (err, docs) {
                if(docs.length > 0 ){
                    ...
                    data["工作完成质量系数"] = "1"
                    console.log("查询id",data["人员"],data["任务编号"])
                }
                else{
        ...
        slibfind.then(
            () => {gzdatalib.insert(data, function (err, docs) {
...
//3 查询所有数据
router.get('/api/gzdata/query', async (ctx, next) => {
    let lib = await new Promise(function (resolve, reject) {
        gzdatalib.find({}).exec(function (err, docs) {
            resolve(docs)
        });
    });
    let c = await new Promise(function (resolve, reject) {
        gzdatalib.count({}, function (err, count) {
            resolve(count)
        });
    });
...
router.get('/api/gzdata/search/', async (ctx, next) => {
    // console.log('收到的查询参数', ctx.request.body)
    console.log('收到的查询参数', ctx.query)//使用ctx.query可以获取？查询的参数
    let lib = await new Promise(function (resolve, reject) {
        gzdatalib.find({ taskid: ctx.query["0"] }).exec(function (err, docs) {
            resolve(docs)
        });
    });
    console.log('列表', lib);
